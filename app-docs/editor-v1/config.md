...
And the validation message is shown in the image below: ![](../../.gitbook/assets/editor-image124.png) Which means the next to add is to add values for the properties listed above. The validation messages states that we need to first include the geodata\_summary summary array inside spatial\_heirarchy
